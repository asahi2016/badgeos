...
class BadgeOS_Report {
	...
	public function render_filters() {
		...
		$output .= '<form method="POST" action="" id="report-filter">';
	...
	public function __get_table_cell_output_value( $data = '', $data_type = '', $row = array() ) {
        ...
        if(isset($_REQUEST['groups']) && !empty($_REQUEST['groups']) && class_exists( 'BP_Groups_Group' ) && bp_is_active( 'groups' )){
            $group_filter = "&groups=".$_REQUEST['groups'];
        }
		...
		switch ( $data_type ) {
			...
			case 'earnings':
				if ( $data )
					$output_value = '<a href="' . admin_url( 'admin.php?page=earnings-report&achievement_id=' . absint( $row['ID'] ) ) . $group_filter.'">' . esc_html( $data ) . '</a>';
				else
					$output_value = $data;
				break;
			default:
